...
%eval(resources(2), test_execution2, true).
...
%eval(car_waiting(1),test_execution3,X).
% EXPECTED BEHAVIOUR -----> test will get success since the car has been washed and is no longer waiting


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TESTS (OLD FASHION FOR MAIN):
% Test 1. Checks if is it possible to 2 consecutive washes in the same station
% test_execution1 :-
%     proc(test_execution,[start_wash(1,2), finish_wash(1,2)]).
%     %indigolog(test_execution).

% % Test 2. Checks if soap is empty after a 2 consecutive washes in the same station with only 1 reload
% test_execution2 :-
%     proc(test_execution22, [start_wash(1,2), finish_wash(1,2), reload(2), start_wash(2,2), finish_wash(2,2)]).
%     %indigolog( [start_wash(1,2), finish_wash(1,2), reload(2), start_wash(2,2), finish_wash(2,2)], ?(resources(2))).

% % Test 3. Checks if after a cycle of wash, the car is still waiting
% test_execution3 :-
%    proc(test_execution33, [start_wash(1,2), finish_wash(1,2), reload_resources(2)]).
%    %indigolog(test_execution33, ?(neg(car_waiting(1)))).
