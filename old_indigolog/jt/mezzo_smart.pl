...
proc(serve_c, pi(n, [?(car_waiting(n)),   pi(m, [?(station_free(m)),serve_car(n,m)])])).
...
proc(serve_c, pi(n, [?(car_waiting(n)),   pi(m, [?(station_free(m)),serve_car_dynamic(n,m)])])).
...
proc(free_station_with_soap, 
    some(n, and(station_free(n), soap(n)))).

proc(serve_car_dynamic2(M),
    pi(n, [
        ?(free_station_with_soap),  % Cerca una stazione libera con sapone
        start_wash(M, n),
        finish_wash(M, n)
    ])
).
proc(serve_car_dynamic3(M,N),
        start_wash(M, N),
        finish_wash(M, N)
).


proc(serve_car_smart, 
    while(free_station_with_soap,pi(n, [?(car_waiting(n)),
        pi(m, [?(station_free(m)),serve_car_dynamic3(n,m)])]))). 
    

proc(control(dynamic_car_wash),
    while(some_cars, serve_car_smart)
).
...
proc(serve_car_smartt(M), 
    if(free_station_with_soap,
        serve_car_dynamicc(M),   % Se esiste una stazione libera con sapone, la usa
        pi(n, [                % Altrimenti trova la prima disponibile e ricarica
            ?(station_free(n)), 
            reload_soap(n), 
            serve_car_dynamicc(n)
        ])
    )
).
% QUELLO CHE "FUNZIONA" NEL SENSO SWITCHA TUTTE LE STAZIONI
proc(control(car_wash_smartt),
    while(some(n, car_waiting(n)),
        pi(n, [
            ?(car_waiting(n)),  % Trova una macchina in attesa
            serve_car_smartt(n)  % Usa la versione intelligente per scegliere la stazione
        ])
    )
).
